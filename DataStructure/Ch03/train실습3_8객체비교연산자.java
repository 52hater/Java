...
class Comp implements Comparator<PhyscData3>{//인터페이스 컴퍼레이터, 피직스데이터3가//클래스명 Comp로 
...
class Compheight implements Comparator<PhyscData3>{//인터페이스 컴퍼레이터, 피직스데이터3가
	public int compare(PhyscData3 p1, PhyscData3 p2) {
		if (p1.height > p2.height > 0) return 1;
		else if (p1.height < p2.height) < 0) return-1;
		else {
			return 0;
		}
		return 1;
...
public class train실습3_8객체비교연산자 {	
	...
	public static void main(String[] args) {
		...
		Arrays.binarySearch(data, key, new CompName);//. 찍을때 보면 제너릭으로 되어있음// new HeightOrderComparator2()
